...
| Published `keep_alive` option | Idle half-open sockets should be detected before the next application operation relies on them. | `keepAliveMs` explicitly enables TCP keepalive with a millisecond initial delay on every pooled PostgreSQL connection. It complements but does not replace managed deadlines. |
...
| [#728](https://github.com/porsager/postgres/issues/728) | PostgreSQL temporal infinity becomes an invalid `Date`. | The default generated/runtime contract preserves both literals with `PgTemporal`; an explicit fail-closed policy rejects them and generates `Date`. |
