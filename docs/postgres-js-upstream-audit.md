...
| Published `keep_alive` option | Idle half-open sockets should be detected before the next application operation relies on them. | `keepAliveMs` explicitly enables TCP keepalive with a millisecond initial delay on every new socket. It complements but does not replace managed deadlines. |
