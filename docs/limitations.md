...
- Self-references and forward references inside `WITH RECURSIVE` are not analysed transitively. Unresolved CTE columns remain conservative and never fall through to equally named physical tables. Ordinary later CTEs can reference earlier CTEs in the same `WITH`. Use `AS "id!"` overrides if recursive output needs an explicit contract.
