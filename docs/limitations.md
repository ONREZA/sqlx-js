...
- The shared wire client reads `sslmode`, `sslrootcert`/`sslcert`/`sslkey`, `application_name`, `options`, `role`, `connect_timeout`, and `statement_timeout` from `DATABASE_URL`. Pool sizing, TCP keepalive, and connection retirement use `createSqlClient(...)` options; `keepAliveMs` only detects idle half-open sockets, `statementTimeoutMs` maps to a per-connection server timeout, and `operationTimeoutMs` bounds the managed end-to-end path.
