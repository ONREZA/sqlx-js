...
- pgschema's upstream connection API has one host field. sqlx-js therefore rejects a distinct `hostaddr` and `host` for every TLS mode before starting pgschema; `sslmode=disable` is available only for a trusted path. The built-in wire paths and `pg_dump` preserve the separate TCP and TLS identities.
