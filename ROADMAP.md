...
| pgschema snapshot and migration handoff | 6 | Desired-state `db dev` / `db verify` now validates application SQL against `schema.sql` in disposable shadow databases. Next, document and automate snapshot handoff for teams migrating from built-in migrations without creating two DDL authorities. |
