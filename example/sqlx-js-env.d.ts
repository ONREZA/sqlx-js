...
export interface SqlxJsGeneratedQueries<TemporalProvider extends import("@onreza/sqlx-js").TemporalApi = import("@onreza/sqlx-js").GlobalTemporalApi> {
  ...
  "INSERT INTO users (name, email, age) VALUES ($1, $2, $3) RETURNING id AS \"id!\", created_at AS \"created_at!\"": { params: [string, string, number | null]; row: { "id": bigint; "created_at": import("@onreza/sqlx-js").PgTimestamptz<TemporalProvider> } };
  ...
  "INSERT INTO users (name, email, settings) VALUES ($1, $2, $3) RETURNING id AS \"id!\"": { params: [string, string, import("@onreza/sqlx-js").SqlxJson<SqlxJsJson.UserSettings>]; row: { "id": bigint } };
  ...
  "SELECT\n     id AS \"id!\",\n     name AS \"name!\",\n     email AS \"email!\",\n     role AS \"role!\",\n     created_at AS \"createdAt!\"\n   FROM public.list_users(\n     COALESCE($role, NULL::public.user_role),\n     COALESCE($search, NULL::text),\n     COALESCE($afterId, NULL::bigint),\n     $limit\n   )": { params: { "role": "admin" | "editor" | "viewer" | null; "search": string | null; "afterId": bigint | null; "limit": number }; row: { "id": bigint; "name": string; "email": string; "role": "admin" | "editor" | "viewer"; "createdAt": import("@onreza/sqlx-js").PgTimestamptz<TemporalProvider> } };
  "SELECT 1 AS one, 'literal'::text AS msg, now() AS ts FROM users LIMIT 1": { params: []; row: { "one": number; "msg": string; "ts": import("@onreza/sqlx-js").PgTimestamptz<TemporalProvider> } };
  ...
  "SELECT id, name, settings FROM users WHERE id = $1": { params: [bigint]; row: { "id": bigint; "name": string; "settings": import("@onreza/sqlx-js").SqlxJson<SqlxJsJson.UserSettings> } };
  "SELECT id, settings FROM users WHERE settings = $1 LIMIT 1": { params: [import("@onreza/sqlx-js").SqlxJson<SqlxJsJson.UserSettings>]; row: { "id": bigint; "settings": import("@onreza/sqlx-js").SqlxJson<SqlxJsJson.UserSettings> } };
  "SELECT id, title, meta, attachments FROM posts WHERE id = $1": { params: [bigint]; row: { "id": bigint; "title": string; "meta": import("@onreza/sqlx-js").SqlxJson<SqlxJsJson.PostMeta> | null; "attachments": (import("@onreza/sqlx-js").SqlxJson<SqlxJsJson.Attachment> | null)[] } };
  ...
  "UPDATE users SET settings = $1 WHERE id = $2 RETURNING id AS \"id!\", settings": { params: [import("@onreza/sqlx-js").SqlxJson<SqlxJsJson.UserSettings>, bigint]; row: { "id": bigint; "settings": import("@onreza/sqlx-js").SqlxJson<SqlxJsJson.UserSettings> } };
}
