...
export interface SqlxJsGeneratedQueries {
  ...
  "INSERT INTO users (name, email, age) VALUES ($1, $2, $3) RETURNING id AS \"id!\", created_at AS \"created_at!\"": { params: [string, string, number | null]; row: { "id": bigint; "created_at": import("@onreza/sqlx-js").PgTimestamptz } };
  ...
  "SELECT\n     id AS \"id!\",\n     name AS \"name!\",\n     email AS \"email!\",\n     role AS \"role!\",\n     created_at AS \"createdAt!\"\n   FROM public.list_users(\n     COALESCE($role, NULL::public.user_role),\n     COALESCE($search, NULL::text),\n     COALESCE($afterId, NULL::bigint),\n     $limit\n   )": { params: { "role": "admin" | "editor" | "viewer" | null; "search": string | null; "afterId": bigint | null; "limit": number }; row: { "id": bigint; "name": string; "email": string; "role": "admin" | "editor" | "viewer"; "createdAt": import("@onreza/sqlx-js").PgTimestamptz } };
  "SELECT 1 AS one, 'literal'::text AS msg, now() AS ts FROM users LIMIT 1": { params: []; row: { "one": number; "msg": string; "ts": import("@onreza/sqlx-js").PgTimestamptz } };
