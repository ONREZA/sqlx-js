...
│   ├── error-catalog.ts      Static PL/pgSQL exception cache + as-const module generation
...
### Working with RLS context

Declare unique lower-case custom setting names such as `app.tenant_id` on the database profile and require their complete values at the transaction boundary. Apply transaction characteristics before context because PostgreSQL requires `SET TRANSACTION` before ordinary statements. Keep context parameterized and transaction-local; never install tenant/user state with session-level `SET` on a pooled connection. Policies that cast a custom setting should use `NULLIF(current_setting('app.key', true), '')` so a reset setting fails closed instead of raising a cast error.

Keep schema ownership and runtime roles separate. RLS-aware query typing and diagnostics belong here; policy generation, automatic tenant predicates, SQL rewriting, and application authorization models do not.
